...
// const BroadCastMessaging = ({route, navigation}) => {
...
const BroadCastMessaging = () => {
  ...
  const {broadcastMessageIDs} = useContext(AdminContext);
  ...
  useEffect(() => {
    console.log(broadcastMessageIDs);
    // updateUser.update({
    //   unreadMessages: 0,
    // });
    getMessages();
  ...
  const onSend = (messages) => {
    ...
    const sendPath = firestore()
      .collection('Users')
      .doc(user.uid)
      .collection('UsersMessages');

    broadcastMessageIDs.map((id) => {
      sendPath.doc(id).collection('Messages').add(messages[0]);
      sendPath.doc(id).update({
