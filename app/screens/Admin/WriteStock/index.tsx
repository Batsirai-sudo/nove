...
import {TextComponent, SearchBar, Datatable, Header} from '@components';
...
const WriteStock = () => {
  ...
  const {getProducts, myshopDataArray} = useContext(AdminContext);
  ...
  const calculateTotalBuyingAmount = () => {
    const x = fullData;
    const value = x.map((d) => Number(d.value) * Number(d.salePrice));
