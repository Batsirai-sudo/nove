...
const Index = (props) => {
  ...
  const {createBrand, getBrands, getShopId_Data} = useContext(CommonContext);
  ...
  const onCreateCategory = () => {
    setModal(true);
    createBrand({name: productValue, id: categories.length + 1})
      .then(() => {
        Errors({
          message: 'successfully created',
          type: 'success',
          description: 'You have created a category successfully',
          // floating: true,
          position: 'bottom',
        });
        fetchCategory();
  ...
  const Title = () => {
    return <ModalTitle textStyle={styles.title} title="Create Brand" />;
  };
