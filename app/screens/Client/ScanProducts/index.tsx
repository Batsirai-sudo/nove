...
const ScanProducts = memo(() => {
  ...
  const onBarCodeReads = useCallback((barcodes) => {
    ...
    if (JSON.parse(data.data).errorCode === '3') {
      ...
      setData({...data, modalVisible: true, content: data.data});
    }
  });
  ...
  return (
    <View
      ...
      <Dialog
        ...
        title={data.title}
        firstButtonOnPress={() => setData({...data, modalVisible: false})}
        onTouchOutside={() => setData({...data, modalVisible: false})}
        onSwipefunc={() => setData({...data, modalVisible: false})}
