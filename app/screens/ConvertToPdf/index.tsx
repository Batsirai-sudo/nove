...
import { batchTable,InvoiceTable } from '@config';
import {
	SaveButton,
	Icon,
	Input,
...
const ConvertToPdf = (props) => {
	...
	const onCreate = async () => {
		...
		let options2 = {
			html: InvoiceTable(props.route.params.currentFromHomeOrders,props.route.params.orderUser),
			fileName: 'test',
		};
		...
		<View style={styles.container}>
			{loading ? (
				<View style={styles.loading}>
					<AwesomeLoading indicatorId={11} size={70} isActive={true} text="Generating PDF" />
				</View>
			) : (
				<>
					<Pdf
					...
					<View style={styles.floatText}>
						<TextComponent style={{ color: 'white' }}>{currentPage}</TextComponent>
					...
					<View
						style={[
							styles.floatButton,
							{
								backgroundColor: horizontal ? 'rgba(240,95,62,0.9)' : 'rgba(0,0,0,0.6)',
								bottom: 300,
							},
						]}
					>
						<TouchableOpacity onPress={() => (horizontal ? setHorizontal(false) : setHorizontal(true))}>
