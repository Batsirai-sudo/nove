...
const PreviewOrder = () => {
  ...
  const {requestOrder, sendShopOrder, storeClientsOrder} = useContext(
    CommonContext,
  );
  const [modal, setModal] = useState(false);
  const [data] = useState(storeClientsOrder);

  const renderItem = useCallback(({item}) => {
    const {quantity, name, amount, mass, InCase} = item;
    console.log(item);
    return (
      <CreateOrderPreviewItem
        titlePost={name}
  ...
  return (
    <View style={{height: '100%'}}>
      <CustomProgressBar
        ...
        visible={modal}
      />
      <HeaderReward amount={data.totalAmount} />
