...
const Index = () => {
	...
	const { getShopDeliveries, getMySelectedOrders } = React.useContext(CommonContext);
	...
	const containsQuery = ({ invoice, fullName }, query) => {
		if (invoice.toLowerCase().includes(query) || fullName.toLowerCase().includes(query)) {
			return true;
		}
	...
	const HotKeysdata = ['oscar', 'new york fashion show', 'night party', 'lux bar party'];
	const onOpen = () => {
		...
		<View style={{ height: height_screen }}>
			<View>
				...
				{!value ? (
					...
					<ShimmerLoading style={styles.item} Component={ShimmerItemOrder} height={shimmerItemOrderHeight} />
				) : (
					<FlatList
